...
func (s *Server) addRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("/api/v1/prices/cities", s.handleFetchPricesCities)
	mux.HandleFunc("/api/v1/prices/sync", s.handleSyncPricesCity)
...
func (s *Server) handleTransactions(w http.ResponseWriter, r *http.Request) {
	transactions, err := s.pricesAPI.GetAllTransactions(r.Context(), "Helsinki")
	if err != nil {
		s.logger.ErrorContext(r.Context(), "fetch all transactions from prices", "err", err)
...
func (s *Server) handleListTransactions(w http.ResponseWriter, r *http.Request) {
	...
	response := make([]PricesResponse, len(transactions))
...
func (s *Server) handleFetchPricesCities(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
		return
	}

	cities, err := s.pricesAPI.FetchCities(r.Context())
	if err != nil {
		s.logger.ErrorContext(r.Context(), "fetch cities from prices", "err", err)
...
func (s *Server) handleSyncPricesCity(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.pricesSync.SyncCity(r.Context(), city); err != nil {
