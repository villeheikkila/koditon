...
func (s *SyncService) SyncCity(ctx context.Context, city string) error {
	...
	cityRow, err := s.queries.UpsertPricesCity(ctx, city)
	if err != nil {
	...
	if len(postalCodes) > 0 {
		...
		rows, err := s.queries.UpsertPricesPostalCodesBulk(ctx, db.UpsertPricesPostalCodesBulkParams{
			Codes:  postalCodes,
			CityID: cityID,
		})
		if err != nil {
	...
	if len(neighborhoods) > 0 {
		...
		rows, err := s.queries.UpsertPricesNeighborhoodsBulk(ctx, db.UpsertPricesNeighborhoodsBulkParams{
			Names:  neighborhoods,
			CityID: cityID,
		})
		if err != nil {
	...
	if len(transactions) > 0 {
		...
		rowsAffected, err := s.queries.UpsertPricesTransactionsBulk(ctx, *params)
		if err != nil {
...
func (s *SyncService) buildTransactionParams(transactions []*TransactionEntity, neighborhoodIDs map[string]pgtype.UUID) (*db.UpsertPricesTransactionsBulkParams, error) {
