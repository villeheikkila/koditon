...
func shortcutAdBuilding(row db.GetShortcutAdUnifiedDetailRow, payload rawMap, location Location) BuildingDetails {
	identity := computedBuildingIdentity("shortcut", "building", ptrUUIDString(row.ShortcutBuildingID), location, valueOrEmpty(row.ShortcutBuildingHousingCompany), "", strconv.FormatInt(row.ShortcutBuildingExternalID, 10))
