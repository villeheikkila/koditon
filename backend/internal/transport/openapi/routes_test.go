...
func TestAddRoutesIncludesAddressLookup(t *testing.T) {
	...
	for _, want := range []string{"raw_transactions", "linked_to_lookup", "candidate_to_lookup", "scope", "is_matched", "matched_listing_count", "source_records", "source_candidates", "candidate_offering_id", "transactions", "link_type", "external_url_available", "reasons_summary", "canonical_id"} {
...
func TestAddRoutesIncludesTransactionMatchReview(t *testing.T) {
	...
	doc := string(data)
	for _, want := range []string{"/api/v1/property-model/transaction-match-candidates", "transaction-match-candidates", "Review prices matches", "candidate plus linked rows", "price_delta_percent", "listing", "transaction", "canonical_id", "offering_id", "native_id", "external_url_available"} {
		if !strings.Contains(doc, want) {
