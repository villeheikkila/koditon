...
func TestAddRoutesIncludesTransactionMatchReview(t *testing.T) {
	...
	doc := string(data)
	for _, want := range []string{"/api/v1/property-model/transaction-match-candidates", "transaction-match-candidates", "price_delta_percent", "listing", "transaction", "canonical_id", "offering_id", "native_id", "external_url_available"} {
		if !strings.Contains(doc, want) {
