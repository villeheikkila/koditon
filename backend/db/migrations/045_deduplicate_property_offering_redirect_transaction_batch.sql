...
BEGIN
    ...
    ALTER TABLE public.property_offering_sources DISABLE TRIGGER trg_refresh_housing_company_geom_for_source;
    ...
    ALTER TABLE public.property_offering_sources ENABLE TRIGGER trg_refresh_housing_company_geom_for_source;
    ...
    SELECT DISTINCT ON (r.target_property_offering_id, pot.prices_transaction_id)
    ...
    ORDER BY r.target_property_offering_id, pot.prices_transaction_id, pot.property_offering_transaction_link_score DESC, pot.property_offering_transaction_updated_at DESC
    LIMIT GREATEST(batch_limit, 1);
    ...
    USING public.property_offering_redirects r
    WHERE r.source_property_offering_id = pot.property_offering_id
        AND EXISTS (
            SELECT 1
            FROM tmp_property_offering_transaction_redirect_moves moves
            WHERE moves.target_property_offering_id = r.target_property_offering_id
                AND moves.prices_transaction_id = pot.prices_transaction_id
        );
