...
WHERE (sqlc.narg('transaction_id')::uuid IS NOT NULL OR latest.status = ANY(ARRAY['candidate'::text, 'ambiguous'::text]))
    ...
    AND (sqlc.narg('postal')::text IS NULL OR NULLIF(regexp_replace(trim(COALESCE(doc.postal, '')), '[^0-9]+', '', 'g'), '') = NULLIF(regexp_replace(trim(COALESCE(sqlc.narg('postal')::text, '')), '[^0-9]+', '', 'g'), ''))
