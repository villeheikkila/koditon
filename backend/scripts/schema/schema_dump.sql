...
with tables as (
  ...
  where n.nspname in ('public', 'runtime')
...
schema_statements as (
  select
    0 as sort_order,
    s.schema_name as object_name,
    'CREATE SCHEMA IF NOT EXISTS ' || quote_ident(s.schema_name) || E';\n' as statement
  from (
...
select
  statement
from (
  select * from schema_statements
  union all
  select * from table_statements
) statements
