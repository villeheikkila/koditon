...
func TestScreenSnapshots(t *testing.T) {
	...
	city := newCityPickerScreen(ctx, ctx.subsystems[3].Actions[2], nil, "Subsystems > Prices > Actions")
	city.Resize(100, 34)
	searchForm := newTransactionsSearchFormScreen(ctx, ctx.subsystems[3].Actions[2], []string{"Helsinki"}, "Subsystems > Prices > Actions")
